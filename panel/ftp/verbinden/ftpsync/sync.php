...
$ftp_server1 = $_SESSION["ftpserver1"];
$port = $_SESSION["port1"];
$conn_id = ftp_connect($ftp_server1,$port);
...
/*   echo("<table style='background: #0b3e75;border-radius:8px;-moz-border-radius:5px;-webkit-border-radius:5px;'>");
  ...
  {
      echo "<tr><td> $dirname  </td><td>" . $dirinfo['chmod'] . " </td><td> " . $dirinfo['owner'] . " </td><td> " . $dirinfo['group'] . " </td><td></td><td> " . $dirinfo['month'] . " </td><td> " . $dirinfo['day'] . " </td><td> " . $dirinfo['time'] . "</td>
	  </tr>";
	  $teller++;
  }
...
<form method="post" action="adir.php" class="form-inline">
									...
									<?php echo "	<input type='hidden' name='ftp' value='$ftp_server1'>
	  <input type='hidden' name='user' value='$ftp_user_name'>
	  <input type='hidden' name='pass' value='$ftp_user_pass'>
	  <input type='hidden' name='port' value='$port'> "; ?>
										<input  class="form-control" id="exampleInputEmail2" name="dirname" placeholder="map toevoegen">
									</div>
